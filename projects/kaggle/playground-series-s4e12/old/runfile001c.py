...
def create_agg_features(cat_col, num_col):
    agg_stats = X.groupby(cat_col)[num_col].agg(['mean', 'std', 'min', 'max']).rename(
        columns={
            'mean': f"{cat_col}_{num_col}_mean",
            'std': f"{cat_col}_{num_col}_std",
            'min': f"{cat_col}_{num_col}_min",
            'max': f"{cat_col}_{num_col}_max",
        }
    ).reset_index()
    
    X_merged = X.merge(agg_stats, on=cat_col, how='left')
    test_merged = test_features.merge(agg_stats, on=cat_col, how='left')
    
    # Update global DataFrames
    global X, test_features
    X = X_merged
    test_features = test_merged
    
    # Collect feature names
    current_mean = f"{cat_col}_{num_col}_mean"
    mean_features.append(current_mean)
    current_agg_features = [f"{cat_col}_{num_col}_{stat}" for stat in ["mean", "std", "min", "max"]]
    agg_feature_names.extend(current_agg_features)
    
    # EWMA calculations using groupby and transform
    for alpha in decay_factors:
        ewma_name = f"{cat_col}_{num_col}_ewma_{alpha}"
        
        # Calculate EWMA per group
        X[ewma_name] = X.groupby(cat_col)[num_col].transform(lambda x: x.ewm(alpha=alpha).mean().shift(1))
        test_features[ewma_name] = test_features.groupby(cat_col)[num_col].transform(lambda x: x.ewm(alpha=alpha).mean().shift(1))
...
# Create all combinations of cat_cols and important_num_cols
tasks = [(cat_col, num_col) for cat_col in cat_cols for num_col in important_num_cols]

# Parallel processing of aggregation features
Parallel(n_jobs=num_cores)(
    delayed(create_agg_features)(cat_col, num_col) for cat_col, num_col in tasks
)
...
# Step 14/22: Dodawanie cech polinomialnych
...
X_agg_scaled = pd.DataFrame(scaler_agg.fit_transform(X_agg), columns=agg_feature_names)
test_agg_scaled = pd.DataFrame(scaler_agg.transform(test_agg), columns=agg_feature_names)
