...
# Krok 10: Tworzenie cech binowanych dla ważnych kolumn numerycznych
print_step("Tworzenie cech binowanych dla ważnych kolumn numerycznych")
# Ustawienie n_bins=63 zgodnie z dokumentem
binner = KBinsDiscretizer(n_bins=63, encode="onehot-dense", strategy="quantile")
X_binned = binner.fit_transform(X[important_num_cols])
test_binned = binner.transform(test_features[important_num_cols])

# Dynamiczne generowanie nazw cech binowanych
binned_feature_names = []
for i, col in enumerate(important_num_cols):
    bins = binner.n_bins_[i]  # Rzeczywista liczba binów dla kolumny
    binned_feature_names.extend([f"{col}_bin_{j}" for j in range(bins)])

# Sprawdzenie zgodności liczby kolumn
if X_binned.shape[1] != len(binned_feature_names):
    raise ValueError(f"Niezgodność liczby kolumn binowanych: {X_binned.shape[1]} vs {len(binned_feature_names)}")
...
# Dynamiczne pobieranie nazwy skryptu bez rozszerzenia .py
script_name = os.path.splitext(os.path.basename(__file__))[0]

# Ścieżka do pliku journal
journal_path = './optuna-study-' + script_name + '.journal'
...
# Krok 19: Uruchamianie optymalizacji hiperparametrów za pomocą Optuna w pętli
