...
def fit_transform(
    ...
    submodule_dir = artifacts.get_submodule_artifact_dir(
        artifact_dir, "time_series_features"
    )
    ...
    if config.get("use_original_features_only"):
        ...
        if orig_features:
            value_cols = dataframe_utils.filter_original_columns(
                value_cols, orig_features
            )
    ...
    if missing_values:
        return (
            ...
            test_df,
            orig_df,
            {"error": f"Missing value_cols: {missing_values}"},
        )
    ...
    if config["entity_id_col"]:
        ...
        if missing_groups:
            return (
                ...
                test_df,
                orig_df,
                {"error": f"Missing entity_id_col: {missing_groups}"},
            )
    ...
    if ts_col and ts_col not in train_df.columns:
        return (
            ...
            test_df,
            orig_df,
            {"error": f"Missing timestamp_col: {ts_col}"},
        )
        ...
        for window in config["windows"]:
            for agg in config["rolling_aggs"]:
                ...
                if grp_cols:
                    ...
                    combined[name] = res.reset_index(
                        level=list(range(len(grp_cols))), drop=True
                    )
    ...
    if new_features:
        if fill == "zero":
            ...
            if grp_cols:
                combined[new_features] = combined.groupby(grp_cols)[
                    new_features
                ].ffill()
            else:
    ...
    train_out = combined[combined["_ts_split"] == "train"].drop(
        columns=["_ts_split"], errors="ignore"
    )
    test_out = combined[combined["_ts_split"] == "test"].drop(
        columns=["_ts_split"], errors="ignore"
    )

    val_out = None
    if val_df is not None:
        val_out = combined[combined["_ts_split"] == "val"].drop(
            columns=["_ts_split"], errors="ignore"
        )
    ...
    if config["drop_original_value_cols"]:
        ...
        if val_df is not None:
            val_df = val_df.drop(columns=value_cols)
