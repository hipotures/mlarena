...
class ActionOracle:
    ...
    def _load_model(self):
        try:
            ...
            if self.predictor.problem_type != "binary":
                logger.warning(
                    f"Oracle model problem type is {self.predictor.problem_type}, expected 'binary'. Predictions might be incorrect."
                )
            ...
            logger.error(
                f"Failed to load ActionOracle model from {self.model_path}: {e}"
            )
            self.predictor = None
        ...
        try:
            ...
            if csv_path.exists():
                ...
                if not hasattr(self, "feature_schema"):
                    logger.warning(
                        f"Schema file {csv_path} not found. Fallback to predictor metadata."
                    )
                    self.feature_schema = (
                        self.predictor.feature_metadata_in.get_features()
                    )
        except Exception as e:
            logger.warning(
                f"Failed to load CSV schema: {e}. Fallback to predictor metadata."
            )
    ...
    def evaluate_actions(
        self, parent_node: Any, candidate_actions: List[Any]
    ) -> Tuple[List[Any], List[float]]:
        """
        Evaluates a list of candidate actions.
        """
        if not self.enabled or not candidate_actions:
            return candidate_actions, [1.0 / max(1, len(candidate_actions))] * len(
                candidate_actions
            )
        ...
        try:
            ...
            if self.predictor is None:
                ...
                return candidate_actions, [1.0 / len(candidate_actions)] * len(
                    candidate_actions
                )
            else:
                # If we have an old predictor, keep using it instead of failing
                logger.warning(
                    f"Oracle: Failed to reload model (using cached version): {e}"
                )
        ...
        scored_actions = sorted(
            zip(candidate_actions, priors), key=lambda x: x[1], reverse=True
        )
        ...
        if max_actions > 0:
            ...
            logger.info(
                f"[ORACLE] Top-{max_actions} Strategy. Evaluated {len(candidate_actions)}. Kept: {len(kept_pairs)}. Best: {best_desc} (P={best_prob:.4f})"
            )
            ...
            for action, prob in scored_actions:
                ...
                if self.dry_run:
                    ...
                    logger.info(
                        f"[ORACLE DRY-RUN] Action '{act_desc}' P={prob:.4f} -> Would {decision} (Threshold={self.threshold})"
                    )
            ...
            logger.info(
                f"[ORACLE] Threshold Strategy. Evaluated {len(candidate_actions)}. Pruned: {n_pruned} ({n_pruned / len(candidate_actions):.1%}) below threshold {self.threshold}"
            )
        ...
        if not accepted_actions and candidate_actions:
            logger.warning(
                "[ORACLE] All actions were pruned! Falling back to original list."
            )
    ...
    def _prepare_features(
        ...
        prev_action_dict = (
            prev_action.to_record()
            if prev_action and hasattr(prev_action, "to_record")
            else {}
        )
    ...
    def _parse_action(
        self, action_dict: Dict[str, Any], prefix: str = ""
    ) -> Dict[str, Any]:
        if not action_dict:
            return {}
        flat = {}
        group = action_dict.get("group_name") or action_dict.get("group", "unknown")
        variant = action_dict.get("variant_name") or action_dict.get(
            "variant", "unknown"
        )
