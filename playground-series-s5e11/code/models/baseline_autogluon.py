...
def parse_args():
    parser = argparse.ArgumentParser(description="AutoGluon baseline for S5E11")
    parser.add_argument(
        "--skip-submit",
        action="store_true",
        help="Do not run the Kaggle submission workflow",
    )
    parser.add_argument(
        "--auto-submit",
        action="store_true",
        help="Submit to Kaggle without asking for confirmation",
    )
    parser.add_argument(
        "--kaggle-message",
        help="Custom Kaggle submission message",
    )
    parser.add_argument(
        "--wait-seconds",
        type=int,
        default=30,
        help="Seconds to wait after Kaggle submit before scraping score",
    )
    parser.add_argument(
        "--cdp-url",
        default="http://localhost:9222",
        help="Playwright CDP URL (Chrome with --remote-debugging-port)",
    )
    parser.add_argument(
        "--skip-score-fetch",
        action="store_true",
        help="Skip Playwright scraping of the latest public score",
    )
    parser.add_argument(
        "--skip-git",
        action="store_true",
        help="Do not stage/commit changes automatically",
    )
...
def main(args):
    ...
    console.print(Panel.fit(
        "[bold]Tip:[/bold] Run with --auto-submit to skip confirmation or "
