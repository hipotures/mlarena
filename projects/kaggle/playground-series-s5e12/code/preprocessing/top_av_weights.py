...
# Import categorical utils for restoring categorical dtypes from chain
# Go up from project to repo root: av_weights.py -> ... -> /home/xai/ml/kaggle/
REPO_ROOT = Path(__file__).resolve().parents[5]  # Up 5 levels from av_weights.py
sys.path.insert(0, str(REPO_ROOT / "config" / "code" / "preprocessing"))
try:
    from categorical_utils import restore_categorical_types_from_chain
    print(f"✓ Loaded categorical_utils from {REPO_ROOT / 'config' / 'code' / 'preprocessing'}")
except ImportError as e:
    # Fallback: no categorical restoration
    print(f"⚠ Failed to import categorical_utils: {e}")
    restore_categorical_types_from_chain = None
...
def fit_transform(
    ...
    # Restore categorical dtypes from previous preprocessing step
    ...
    for col in CATEGORICAL_COLUMNS:
        if col in train_df.columns:
            ...
            print(f"  ✓ train_df[{col}] → category")
        if col in test_df.columns:
            test_df[col] = test_df[col].astype('category')
            print(f"  ✓ test_df[{col}] → category")
