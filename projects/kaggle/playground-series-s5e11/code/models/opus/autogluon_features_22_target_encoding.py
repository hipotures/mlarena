    ...
    if is_train and 'loan_paid_back' in enriched.columns:
        for col in cols_to_encode:
            ...
            for cat in enriched[col].unique():
                ...
                n_positive = enriched.loc[mask, 'loan_paid_back'].sum()
                n_negative = (~enriched.loc[mask, 'loan_paid_back']).sum()
                
                # Avoid log(0) by adding smoothing
                n_positive = max(n_positive, 0.5)
                n_negative = max(n_negative, 0.5)
                
                total_positive = enriched['loan_paid_back'].sum()
                total_negative = (~enriched['loan_paid_back']).sum()
...
def train(
    ...
    result = base_model.train(train_df, val_df, config, artifacts)
...
def predict(
    ...
    if artifacts and 'target_encoder' in artifacts:
