...
class MLArenaStepWrapper(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, step_name: str, step_idx: int, config: Dict, context, loader_instance
    ):
    ...
    def transform(self, X: MLArenaDataContainer) -> MLArenaDataContainer:
        ...
        if self.context.config and getattr(self.context.config, "json_output", False):
            is_quiet = True
            invocation_params = (
                getattr(self.loader_instance, "invocation_params", {}) or {}
            )
            for key in (
        ...
        if template_name and not template_cfg.get("module"):
            ...
            loader = TemplateLoader(
                self.context.project_root, template_type="preprocess"
            )
        ...
        # 2. Load module
        try:
            preprocess_module = self.loader_instance._load_preprocessing_module(
                module_name
            )
            ...
            preprocess_config["_dataset"] = {
                ...
                "ignored_columns": getattr(
                    self.context.config_module, "IGNORED_COLUMNS", []
                ),
                "problem_type": getattr(
                    self.context.config_module, "AUTOGLUON_PROBLEM_TYPE", "binary"
                ),
            }
            # Pass aggregated original features - Ensure correct lookup
            orig_feat_list = X.state.get("custom_module_state", {}).get(
                "original_features"
            )
            ...
            if supports_eval_df:
                ...
                test_len = len(X.test)
                if X.eval is not None:
                    combined_test = pd.concat([X.test, X.eval], axis=0).reset_index(
                        drop=True
                    )
            ...
            new_train, new_val, new_test, new_eval, new_orig = (
                None,
                None,
                None,
                None,
            ...
            if len(result) == 6:
                ...
                new_train, new_val, combined_res, new_orig, custom_preprocess_state = (
                    result
                )
            ...
            if new_train is not None:
                X.train = new_train.reset_index(drop=True)
            if new_test is not None:
                X.test = new_test.reset_index(drop=True)
            if new_val is not None:
                X.val = new_val.reset_index(drop=True)
            if new_eval is not None:
                X.eval = new_eval.reset_index(drop=True)
            if new_orig is not None:
                X.orig = new_orig.reset_index(drop=True)
            ...
            step_state_data = {
                ...
                "status": "completed",
                "modules": {
                    "preprocess": {"status": "completed", "payload": step_payload}
                },
            }
            ...
            is_quiet = False
            if self.context.config and getattr(
                self.context.config, "json_output", False
            ):
                is_quiet = True
            console = Console(
                file=sys.__stdout__,
...
class PreprocessModule(BaseModule):
    ...
    def can_run(self) -> tuple[bool, str]:
        ...
        if self.invocation_params.get(
            "preprocess_template_config"
        ) or self.invocation_params.get("preprocess_template_path"):
            return True, ""
        ...
        if isinstance(template_name, list):
            return (
                False,
                f"preprocess_template should be string, got list: {template_name}",
            )
    ...
    def _apply_template(
        self, df: pd.DataFrame, template: Dict[str, Any]
    ) -> pd.DataFrame:
    ...
    def _load_preprocessing_module(self, module_name: str):
        ...
        )  # src/mlarena/modules/preprocess.py -> ../../.. -> repo root
    ...
    def _resolve_original_features(
        ...
        if not orig_features:
            init_state = (
                self.context.project_root / "experiments" / "init" / "state.json"
            )
    ...
    def execute(self) -> ModuleResult:
        ...
        chain_exp_id = self.invocation_params.get(
            "chain_exp_id", f"pre-{template_name}"
        )
        ...
        for key in (
            ...
            if (
                key in self.invocation_params
                and self.invocation_params.get(key) is False
            ):
                quiet_console = False
                if json_output:
                    use_stderr = True
        ...
        if cache_ok and processed_train.exists() and processed_test.exists():
            ...
            return ModuleResult(
                ...
                payload={
                    ...
                    "tuning_processed": str(processed_tuning)
                    if tuning_exists
                    else None,
                    "cached": True,
        ...
        if input_source:
            ...
            prev_exp_dir = (
                self.context.project_root / "experiments" / chain_exp_id / input_source
            )
            ...
            if not train_path.exists() or not test_path.exists():
                ...
                return ModuleResult(
                    success=True,
                    payload={"skipped": True, "input_source": input_source},
                    artifacts=[marker],
                )
        ...
        if eval_df is not None:
            combined_test_df = pd.concat([test_df, eval_df], axis=0).reset_index(
                drop=True
            )
        ...
        original_features = self._resolve_original_features(
            train_df, config, prev_custom_state
        )
        ...
        if template_cfg_override:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to load preprocess_template_path: {template_path} ({e})"
                )
        ...
        if pipeline_definition:
            try:
                ...
                if original_features:
                    ...
                    container.state["custom_module_state"]["original_features"] = (
                        original_features
                    )
                ...
                for idx, step_item in enumerate(pipeline_definition):
                    ...
                    wrapper = MLArenaStepWrapper(
                        step_name, idx, step_cfg, self.context, self
                    )
                    pipeline_steps.append((step_name, wrapper))
                    state_steps.append(
                        PipelineStep(name=step_name, module=module_name_for_step)
                    )
                ...
                final_container.train.to_parquet(
                    target_dir / "train_processed.parquet", index=False
                )
                final_container.test.to_parquet(
                    target_dir / "test_processed.parquet", index=False
                )
                ...
                if final_container.eval is not None:
                    ...
                    try:
                        eval_rel = processed_eval.resolve().relative_to(
                    ...
                    final_container.state["custom_module_state"]["eval_path"] = str(
                        eval_rel
                    )
                    final_container.state["custom_module_state"]["eval_rows"] = int(
                        final_container.eval.shape[0]
                    )
                    final_container.state["custom_module_state"]["eval_cols"] = int(
                        final_container.eval.shape[1]
                    )
                ...
                shapes_dict = {
                    ...
                    "val_after": final_container.val.shape
                    if final_container.val is not None
                    else None,
                    "eval_before": final_container.initial_shapes.get("eval"),
                    "eval_after": final_container.eval.shape
                    if final_container.eval is not None
                    else None,
                    "orig_before": final_container.initial_shapes.get("orig"),
                    "orig_after": final_container.orig.shape
                    if final_container.orig is not None
                    else None,
                    "pipeline_mode": True,
                ...
                payload = {
                    "train_processed": str(
                        (target_dir / "train_processed.parquet").resolve()
                    ),
                    "test_processed": str(
                        (target_dir / "test_processed.parquet").resolve()
                    ),
                    "orig_processed": str(
                        (target_dir / "orig_processed.parquet").resolve()
                    )
                    if final_container.orig is not None
                    else None,
                    "tuning_processed": str(
                        (target_dir / "tuning_processed.parquet").resolve()
                    )
                    if final_container.val is not None
                    else None,
                    "eval_processed": str(
                        (target_dir / "eval_processed.parquet").resolve()
                    )
                    if final_container.eval is not None
                    else None,
                    ...
                    "cached": False,
                    "shapes": shapes_dict,
                    "custom_module_state": final_container.state.get(
                        "custom_module_state", {}
                    ),
                ...
                if step_state_path.exists():
                    try:
                        ...
                        if modules:
                            preprocess_module = modules.get("preprocess") or next(
                                iter(modules.values())
                            )
                            step_payload = preprocess_module.setdefault("payload", {})
                            step_custom_state = (
                                step_payload.get("custom_module_state") or {}
                            )
                            step_custom_state.update(
                                final_container.state.get("custom_module_state", {})
                            )
                ...
                artifacts_list = [
                    target_dir / "train_processed.parquet",
                    target_dir / "test_processed.parquet",
                ]
                if final_container.orig is not None:
                    artifacts_list.append(target_dir / "orig_processed.parquet")
                if final_container.val is not None:
                    artifacts_list.append(target_dir / "tuning_processed.parquet")
                if final_container.eval is not None:
                    artifacts_list.append(target_dir / "eval_processed.parquet")

                return ModuleResult(
                    success=True, payload=payload, artifacts=artifacts_list
                )
                ...
                if "container" in locals():
                    shapes_dict = {
                        ...
                        "train_after": container.train.shape
                        if hasattr(container, "train")
                        else None,
                    }
        ...
        if custom_module_name:
            try:
                ...
                # Handle both nested (legacy) and flattened (TemplateLoader) configs
                if "config" in template_cfg and isinstance(
                    template_cfg["config"], dict
                ):
                    preprocess_config = template_cfg["config"].copy()
                else:
                    # Configuration is already flattened
            ...
            if template_cfg:
                ...
                if template_cfg.get("drop_columns"):
                    operations.append(
                        f"drop {len(template_cfg['drop_columns'])} columns"
                    )
                if template_cfg.get("fillna"):
                    operations.append(f"fillna ({len(template_cfg['fillna'])} columns)")
                if operations:
                    console.print(
                        f"[bold]Template operations:[/bold] {', '.join(operations)}"
                    )
        ...
        payload = {
            ...
            "tuning_processed": str(processed_tuning_final)
