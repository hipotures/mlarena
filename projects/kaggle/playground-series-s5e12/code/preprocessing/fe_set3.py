...
Includes GPU-accelerated Permutation Importance Selection to drop harmful features.
...
- Feature Selection Stage 2: Permutation Importance on GPU (Drops features that worsen Validation Score)
...
def _run_permutation_selection_gpu(
    ...
    """
    Stage 2 Selection: Permutation Importance on Holdout Set (GPU).
    Drops features where shuffling yields >= validation score (harmful or useless).
    """
    if not LGBM_AVAILABLE:
        print("LightGBM not installed, skipping Selection.")
        return df.columns, {}
    ...
    # 1. Create Internal Holdout Split (e.g. 80/20) to detect overfitting features
    # (We cannot use the main Val set here to avoid leakage if downstream uses it)
    df_pd = df.to_pandas()
    # Simple shuffle split
    df_pd = df_pd.sample(frac=1.0, random_state=42).reset_index(drop=True)
    split_idx = int(len(df_pd) * 0.8)
    
    train_pd = df_pd.iloc[:split_idx]
    val_pd = df_pd.iloc[split_idx:].copy() # Copy for shuffling

    y_train = train_pd[target]
    X_train = train_pd.drop(columns=[target])
    y_val = val_pd[target]
    X_val = val_pd.drop(columns=[target])
    ...
    cat_meta = {c: X_train[c].cat.categories for c in cat_cols}
    ...
    params = {
        ...
        "verbosity": -1,
        "n_jobs": -1,
        "n_estimators": 200, # Sufficient for detecting signal
        "learning_rate": 0.05,
        "num_leaves": 31,
    }
    ...
    baseline_score = scorer(y_val, baseline_preds)
    print(f"Permutation Selection Baseline ({eval_metric}): {baseline_score:.5f}")
    ...
    # Optimization: Predict on GPU is fast, but looping python is slow.
    # For < 500 features it's fine.
    total = len(features)
    log_every = max(1, total // 10)

    for idx, col in enumerate(features, 1):
        # Save original column (keeps dtype)
        orig_col = X_val[col].copy()

        # Shuffle values (preserve dtype/categories)
        shuffled = orig_col.iloc[np.random.permutation(len(orig_col))].reset_index(drop=True)
        shuffled.index = X_val.index
        if col in cat_meta:
            shuffled = pd.Categorical(shuffled, categories=cat_meta[col])
        X_val[col] = shuffled

        # Score
        shuff_preds = model.predict(X_val)
        shuff_score = scorer(y_val, shuff_preds)

        # Restore
        X_val[col] = orig_col
        
        # Calculate Importance
        # If maximize (AUC): Importance = Baseline - Shuffled
        #   If Shuffled is worse (lower), Imp > 0 (Useful)
        #   If Shuffled is better (higher), Imp < 0 (Harmful)
        
        # If minimize (RMSE): Importance = Shuffled - Baseline
        #   If Shuffled is worse (higher error), Imp > 0 (Useful)
        #   If Shuffled is better (lower error), Imp < 0 (Harmful)
        
        if maximize:
            importance = baseline_score - shuff_score
        else:
            importance = shuff_score - baseline_score
            
        # Decision: Keep if Importance > threshold (usually 0 or small epsilon)
        # Meaning: Removing the info (shuffling) made the model worse.
        # If Importance <= 0, it means model performed SAME or BETTER with noise -> Drop.
        
        if importance > 0.00001: # Small epsilon to drop useless/neutral
            keep_features.append(col)
        else:
            drop_stats.append((col, importance))

        if idx % log_every == 0 or idx == total:
            print(
                f"Permutation Selection progress: {idx}/{total} processed | "
                f"kept={len(keep_features)}, dropped={len(drop_stats)}"
            )
    print(f"Permutation Selection: Dropped {len(drop_stats)} features (harmful/useless). Kept {len(keep_features)}.")
    
    # Sort dropped for logging (worst offenders first - negative importance)
    drop_stats.sort(key=lambda x: x[1]) # Most negative first
    
    final_cols = keep_features + [target]
    stats = {
        "baseline_score": baseline_score,
        "dropped_count": len(drop_stats),
        "worst_harmful_features": drop_stats[:10], # Features that improved score when shuffled
        "useless_features_count": sum(1 for x in drop_stats if abs(x[1]) < 1e-6)
    }
...
def fit_transform(
    ...
    # 3. Stage 2 Selection: Permutation Importance Selection (GPU)
    # Detects and removes features that hurt validation score
    perm_stats = {}
    if cfg.get("selection_enabled", False):
        print("Starting Permutation Importance Selection on GPU...")
