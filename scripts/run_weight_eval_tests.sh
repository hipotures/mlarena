...
uv run python scripts/mla.py -p $PROJECT --model-template test_xgb_only_wt --preprocess-template $PREPROCESS
uv run python scripts/mla.py -p $PROJECT --model-template test_xgb_only_unwt --preprocess-template $PREPROCESS

# Boosting tests (6h each, 50 HPO trials)
uv run python scripts/mla.py -p $PROJECT --model-template test_wt_eval_true --preprocess-template $PREPROCESS
uv run python scripts/mla.py -p $PROJECT --model-template test_wt_eval_false --preprocess-template $PREPROCESS
