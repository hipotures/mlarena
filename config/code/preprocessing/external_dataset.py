"""
External dataset preprocessing module.

Loads and aligns external/original dataset with Kaggle competition data.
Supports column alignment, name mapping, and optional source tracking.

Key difference from dataset_merger:
- Does NOT concatenate train + orig
...
def fit_transform(
    ...
    Config keys:
        orig_path (str): Path to external dataset CSV (required)
    ...
    if column_mapping:
        ...
        missing_cols = set(column_mapping.keys()) - set(orig_df_loaded.columns)
        if missing_cols:
            raise ValueError(
                f"Column mapping references non-existent columns in external dataset: "
                f"{sorted(missing_cols)}\n"
                f"Available columns: {sorted(orig_df_loaded.columns)}"
            )
    ...
    orig_cols = set(orig_df_loaded.columns)
    ...
    if mode == "align":
        ...
        if not common_cols:
            raise ValueError(
                "No common columns found between Kaggle and external datasets. "
        ...
        for col in kaggle_only:
            orig_df_loaded[col] = pd.NA

        # Add missing columns to kaggle train (for consistency, though we don't modify train)
        train_copy = train_df.copy()
        for col in orig_only:
            train_copy[col] = pd.NA
    ...
    if source_flag:
        # Add to train (kaggle=0) - but we're NOT modifying train, just for consistency
        train_aligned[source_flag] = 0
        # Add to orig (external=1)
    ...
    table.add_row("External columns", str(len(orig_cols)))
    ...
    table.add_row("External rows", f"{len(orig_df_loaded):,}")
...
def transform(df: pd.DataFrame, state_dict: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
    ...
    External dataset loading is a training-time operation only.
    """
    return df.copy()
