...
train = train.sample(frac=0.01, random_state=43).reset_index(drop=True)
...
interaction_features_train_df = pd.concat([df.rename(f"interaction_{feat1}_{feat2}") for df, (feat1, feat2) in zip(interaction_features_train, combinations(mean_features, 2))], axis=1)
interaction_features_test_df = pd.concat([df.rename(f"interaction_{feat1}_{feat2}") for df, (feat1, feat2) in zip(interaction_features_test, combinations(mean_features, 2))], axis=1)
