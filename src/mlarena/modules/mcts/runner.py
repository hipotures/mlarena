...
class MCTSRunner:
    def __init__(self, context: ModuleContext, params: Dict[str, Any]):
        ...
        self.study_id, self.is_new_study = self.storage.create_study(
            self.config.study_name, self.direction
        )
        ...
        if not self.is_new_study:
            ...
            if not self.mcts_live:
                ...
                with self.storage._connect() as conn:
                    row = conn.execute(
                        "SELECT value FROM trial_values WHERE trial_id = (SELECT trial_id FROM trials WHERE study_id=? AND number=0)",
                        (self.study_id,),
                    ).fetchone()
                    if row:
                        base_score = row[0]
                ...
                with self.storage._connect() as conn:
                    row = conn.execute(
                        "SELECT metric_name FROM mcts_evaluations WHERE metric_name IS NOT NULL LIMIT 1"
                    ).fetchone()
                    if row and row[0]:
                        metric_name = row[0]
        ...
        self.materializer = TemplateMaterializer(
            run_id=self.run_id, project_root=context.project_root
        )
    ...
    def _log_leaderboard_debug(
        ...
        for alias, model_name, score in items:
            label = (
                f"{alias}:{model_name}" if alias and alias != model_name else model_name
            )
            try:
    ...
    def run(self) -> ModuleResult:
        status_msg = (
            "Starting NEW MCTS Study"
            if self.is_new_study
            else "Resuming EXISTING MCTS Study"
        )

        if not self.mcts_live:
            print(
                f"{status_msg}: {self.config.study_name} (Budget: {self.config.budget})"
            )
        self.logger.info(
            f"{status_msg}: {self.config.study_name} (Budget: {self.config.budget})"
        )
        ...
        best_so_far = (
            -float("inf") if self.direction == StudyDirection.MAXIMIZE else float("inf")
        )
        ...
        def handler(signum, frame):
            ...
            if not self.stop_requested:
                ...
                self.logger.info(
                    "Interrupt requested. Stopping after current iteration."
                )
            else:
                # Second Ctrl+C - force quit
                c.print("\n⚠️ [bold red]Force quitting...[/bold red]")
                if (
                    hasattr(self.executor, "current_proc")
                    and self.executor.current_proc
                ):
                    try:
                        c.print(
                            f"⚠️ Killing current subprocess (PID: {self.executor.current_proc.pid})..."
                        )
        ...
        try:
            ...
            while n_executed < self.config.budget and not self.stop_requested:
                if iteration >= max_iterations:
                    self.logger.warning(
                        f"Reached max safety iterations ({max_iterations}) with only {n_executed} trials. Tree might be fully explored."
                    )
                    break
                ...
                if child == node:
                    ...
                    def _op_key(a):
                        return (
                            getattr(a, "searched_index", None),
                            getattr(a, "step_name", None),
                            getattr(a, "variant_name", None),
                        )
                    ...
                    for ok, v in op_children_visits.items():
                        ...
                        max_tried_sids = max(
                            max_tried_sids, len(op_tried_sids.get(ok, set()))
                        )
                    ...
                    val = (
                        node.value_mean
                        if node.n_visits > 0
                        else self.tree.root.value_mean
                    )
                ...
                with self.storage.atomic() as conn:
                    ...
                    if node.trial_id is not None:
                        self.storage.add_edge(
                            node.trial_id,
                            trial_id,
                            child.action_from_parent.to_record(),
                            conn=conn,
                        )
                ...
                if not fidelity:
                    # Pruned/Done path
                    self.logger.info(
                        f"  -> Trial {trial_number} (ID:{trial_id}): Pruned or done"
                    )
                ...
                self.logger.info(
                    f"I={iteration} (Ts={n_executed + 1}/{self.config.budget}) -> T={trial_number} D={child.state.depth}"
                )
                if self.logger.isEnabledFor(logging.DEBUG):
                    try:
                        self.logger.debug(
                            f"  -> Trial {trial_number} Full Config: {json.dumps(child.state.steps)}"
                        )
                    except Exception:
                        self.logger.debug(
                            f"  -> Trial {trial_number} Full Config (Raw): {child.state.steps}"
                        )

                # Materialize and execute
                trial_templates = self.materializer.materialize(
                    child.state, node_id=trial_id, fixed_steps=self.space.fixed_steps
                )
                result = self._execute_trial_with_templates(
                    trial_templates, trial_id, fidelity
                )
                ...
                if result.success and result.value is not None:
                    ...
                    if feat_lambda > 0:
                        ...
                        if not shapes:
                            shapes = (result.details.get("payload") or {}).get(
                                "shapes", {}
                            )

                        in_cols = (
                            shapes.get("train_before", [0, 13])[1]
                            if shapes.get("train_before")
                            else 13
                        )
                        out_cols = (
                            shapes.get("train_after", [0, 13])[1]
                    ...
                    with self.storage.atomic() as conn:
                        ...
                        self.logger.debug(
                            f"[BACKPROP] Propagating value {final_value:.4f} (original: {result.value:.4f}) from node {trial_id}"
                        )
                        ...
                        self.storage.set_trial_value(
                            trial_id, result.value, conn=conn
                        )  # Keep original score in DB for reporting
                    ...
                    if not self.mcts_live:
                        ...
                        model_alias = self._format_model_alias(
                            raw.get("best_model")
                            or raw.get("best_model_implementation")
                        )
                        value_text = (
                            f"{model_alias}={result.value:.5f}"
                            if model_alias
                    ...
                    target_fid = (
                        self.config.multi_fidelity.levels[-1]["name"]
                        if self.config.multi_fidelity.levels
                        else "F2"
                    )
                    ...
                    if not error_msg:
                        error_text = result.details.get(
                            "stderr", ""
                        ) or result.details.get("stdout", "")
                        error_msg = "\n".join(error_text.strip().split("\n")[-10:])

                    self.logger.error(
                        f"  -> Trial {trial_number} failed. Error: {error_msg}"
                    )
                    ...
                    self._cleanup_templates(
                        trial_templates, fidelity, keep=keep_on_fail
                    )
    ...
    def _persist_node_stats_path(self, node: MCTSNode, conn: sqlite3.Connection):
        ...
        while curr is not None:
            if id(curr) in seen:
                self.logger.error(
                    f"Cycle detected in MCTS parent chain (Node ID: {id(curr)}); aborting stats persist."
                )
                break
            seen.add(id(curr))
    ...
    def _render_tree(self, best_score: float) -> Tree:
        ...
        base_node_text = Text(
            base_label, style="bold blue" if is_base_best else "grey70"
        )
    ...
    def _evaluate_baseline(self):
        ...
        templates = self.materializer.materialize(
            state, node_id=0, fixed_steps=self.space.fixed_steps
        )
        ...
        if result.success and result.value is not None:
            ...
            self.logger.debug(
                f"[BACKPROP] Propagating baseline value {result.value:.4f}"
            )
            ...
            # PERSIST root stats
            self.storage.update_node_stats(
                trial_id,
                self.tree.root.n_visits,
                self.tree.root.value_sum,
                self.tree.root.value_best,
            )
            ...
            error_msg = "Baseline Failed! Check mcts.log for details."
    ...
    def _cleanup_templates(
        self, templates: Dict[str, Any], fidelity: str, keep: bool = False
    ):
        ...
        if not base_name or not fidelity:
            self.logger.warning("Cleanup skipped: empty base_name or fidelity")
            return
    ...
    def _execute_trial_with_templates(
        self, templates: Dict[str, Any], trial_id: int, fidelity: str
    ) -> ExperimentResult:
        base_name = templates["base_name"]
    ...
    def _get_next_fidelity(self, node: MCTSNode, trial_id: int) -> Optional[str]:
        ...
        if not self.config.multi_fidelity.enable or not levels:
            ...
            if any(e["fidelity"] == "F2" for e in evals if e["status"] == "COMPLETE"):
                return None
            return "F2"
        ...
        for i, level in enumerate(levels):
            ...
            if prev_name not in completed_fids:
                return None
            prev_score = next(
                (e["value"] for e in evals if e["fidelity"] == prev_name), None
            )
            ...
            if self._should_promote(prev_score, history, top_frac):
                ...
                self.storage.add_evaluation(
                    trial_id, name, "PRUNED", None, "pruned", 0.0
                )
    ...
    def _should_promote(
        self, value: float, history: List[float], top_fraction: float
    ) -> bool:
        ...
        if self.direction == StudyDirection.MAXIMIZE:
            ...
            if cutoff_idx >= len(valid_history):
                cutoff_idx = len(valid_history) - 1
