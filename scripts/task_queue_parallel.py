...
from mlarena.utils.queue_parallel import TaskQueueParallel
...
class TaskQueueApp(App):
    ...
    def __init__(self, project: str, auto_run: bool = False):
        ...
        self.queue = TaskQueueParallel(self.project_root)
    ...
    def refresh_data(self) -> None:
        ...
        if running_tasks:
            status_msg = "\n".join([f"Task #{t['id']}: {t['command']}" for t in running_tasks])
        else:
            status_msg = "Idle"
            
        if self.is_running_queue and not running_tasks:
    ...
    def worker_run_queue(self) -> None:
        ...
        try:
            max_jobs = int(self.query_one("#max-jobs-select", Select).value or 4)
            self.queue.run_queue(console=capture_console, max_jobs=max_jobs)
        except Exception as e:
