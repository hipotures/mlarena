...
def train(
    ...
    """
    # Prepare data
    drop_cols = [config.dataset.target]
    if config.dataset.id_column in train_df.columns:
        drop_cols.append(config.dataset.id_column)

    X_train = train_df.drop(columns=drop_cols)
    y_train = train_df[config.dataset.target]

    # Cast object columns to category for LightGBM handling
    ...
    legacy_best = legacy_optuna_dir / "best_params_lightgbm.json"
    ...
    if best_params_file.exists():
        ...
        if not config.optuna.study_name:
            config.optuna.study_name = f"lgbm_{config.system.experiment_id}"
        ...
        if "lightgbm" not in config.optuna.param_space:
            raise ValueError("Optuna param_space for 'lightgbm' not found in config.optuna.param_space")
