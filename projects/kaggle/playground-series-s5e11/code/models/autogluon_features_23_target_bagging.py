"""
AutoGluon variant #23: fe21 features + CV target encoding/WOE with bagging/stacking support.

Built to (a) carry the richer feature block from fe22 and (b) actually pass
num_bag_folds/num_stack_levels into TabularPredictor.fit so template settings take effect.
"""
...
VARIANT_NAME = "feature-set-23-target-encoding-bagging"
DEFAULT_CATEGORICAL_COLS = ["grade_subgrade", "loan_purpose", "employment_status", "home_ownership"]

# Stored encoders for inference phase
target_encoder = None
woe_mappings: Dict[str, Dict[Any, float]] = {}


def get_default_config() -> Dict[str, Any]:
    """Baseline defaults; templates override compute budget."""
    return {
        "hyperparameters": {
...
def _engineer_features(
    ...
    """Apply fe21 features + CV target encoding (+ optional WOE)."""
    ...
    n_splits = int(config.model.get("target_encoding_folds", 5))
    smoothing = float(config.model.get("target_encoding_smoothing", 1.0))
...
def preprocess(df: pd.DataFrame, config: ModelConfig, is_train: bool = True) -> pd.DataFrame:
    enable_woe = bool(config.model.get("enable_woe", True))
...
def _build_fit_kwargs(config: ModelConfig) -> Dict[str, Any]:
    ...
    # Use sequential folding by default to avoid Ray/socket permissions in restricted environments
...
def train(
    ...
    # Persist encoders for prediction
...
def predict(
    ...
    # Re-run encoding in case preprocess was skipped by a caller.
    enable_woe = bool(config.model.get("enable_woe", True))
