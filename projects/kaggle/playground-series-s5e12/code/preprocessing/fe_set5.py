...
Includes GPU-accelerated Feature Selection using XGBoost (inference on GPU) and
medical priors (blood pressure metrics, lipid ratios, BMI categories, age interactions).

Highlights:
- Polars-native binning (`cut`/`qcut`), rounding, digit features, pairwise cat combos
- Medical feature block (pulse pressure, MAP, cholesterol ratios, BMI medical bins, age interactions)
- Skew-aware log feature for physical_activity_minutes_per_week
...
def _add_medical_features(df: pl.DataFrame, cat_cols: List[str], num_cols: List[str]) -> tuple[pl.DataFrame, List[str], List[str]]:
    ...
    # 1. Blood pressure metrics and interactions
    ...
    if "hdl_cholesterol" in cols:
        hdl_col = pl.col("hdl_cholesterol")
        if "cholesterol_total" in cols:
            exprs.append(_safe_ratio(pl.col("cholesterol_total"), hdl_col).alias("chol_ratio_total_hdl"))
            new_num.append("chol_ratio_total_hdl")
        if "ldl_cholesterol" in cols:
            exprs.append(_safe_ratio(pl.col("ldl_cholesterol"), hdl_col).alias("chol_ratio_ldl_hdl"))
            new_num.append("chol_ratio_ldl_hdl")
        if "triglycerides" in cols:
            exprs.append(_safe_ratio(pl.col("triglycerides"), hdl_col).alias("chol_ratio_tg_hdl"))
            new_num.append("chol_ratio_tg_hdl")

    # 3. BMI medical categories and age interaction
    if "bmi" in cols:
        exprs.append(
...
def _process_pipeline(
    ...
    for c in skewed_cols:
        if c in df.columns:
            log_col = f"{c}_log1p"
            df = df.with_columns(pl.col(c).log1p().alias(log_col))
            if log_col not in num_cols:
...
def fit_transform(
    ...
    if sample_frac < 1.0:
        print(f"[fe_set5] Sampling train data: frac={sample_frac}")
