...
1.  **Scripts Layer** (`scripts/`): CLI entry point and orchestration.
    -   `mla.py`: The single entry point for orchestrating the modular pipeline. It replaces the legacy scripts (`ml_runner.py`, `optuna_runner.py`, etc.).
2.  **Core Package** (`src/kaggle_tools/` and `src/mlarena`): Shared Python packages.
    -   `src/kaggle_tools`: Contains universal submission creation/validation logic.
    -   `src/mlarena`: The core of the `mla.py` CLI, including the module registry and pipeline executor.
3.  **Project Layer** (`projects/kaggle/[competition-name]/`): Individual competition directories.
    -   `code/utils/config.py`: Competition-specific constants.
    -   `code/models/`: Model implementations.
    -   `code/exploration/`: EDA scripts.
    -   `templates/model.yaml`: Experiment templates.
4.  **Tracking Layer**: Automatic experiment → submission → git linkage.
    -   Every submission created via `mla.py` captures the git hash and a code snapshot.
...
```
This is called by the `mla.py` workflow and automatically:
1.  Detects the calling code path.
2.  Logs the experiment, saving a snapshot.
3.  Tracks the submission, linking it to the experiment and git hash.
...
uv run python scripts/mla.py submissions --project [competition-name] list  # get git hash
...
1.  **Running experiments without committing** → Missing git hash, can't reproduce
    ...
    -   Always `git commit` before running `mla.py`
...
3.  **Calling `mla.py` from wrong directory** → File not found
    -   Tools must be called from repo root: `uv run python scripts/mla.py ...`
