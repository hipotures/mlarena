...
from mlarena.utils.project import (
    data_paths,
    load_project_config,
...
class PredictModule(BaseModule):
    ...
    def execute(self) -> ModuleResult:
        ...
        if not model_entry or not getattr(model_entry, "payload", None):
            ...
            return ModuleResult(
                success=False, error="model not run", artifacts=[marker]
            )
        ...
        id_series = (
            test_df_raw[id_col].copy()
            if id_col and id_col in test_df_raw.columns
            else None
        )
        ...
        if not pp_template:
            pp_template = (model_entry.payload or {}).get("preprocess_template") or (
                model_entry.invocation or {}
            ).get("preprocess_template")  # type: ignore[union-attr]
        if not pp_exp_dir:
            pp_exp_dir = (model_entry.payload or {}).get("preprocess_exp_dir") or (
                model_entry.invocation or {}
            ).get("preprocess_exp_dir")  # type: ignore[union-attr]
        ...
        if pp_template:
            ...
            (
                Path(pp_exp_dir) / "artifacts" / "preprocess" / "test_processed.csv.gz"
                if pp_exp_dir
                else None
            )
        ...
        try:
            ...
            submissions_dir = Path(
                getattr(
                    config, "SUBMISSIONS_DIR", self.context.project_root / "submissions"
                )
            )
            sample_sub_path = Path(
                getattr(
                    ...
                    self.context.project_root / "data" / "sample_submission.csv",
                )
            )
            ...
            console.print(
                f"[yellow]⚠ Warning: Could not create unified submission file in {submissions_dir}: {e}[/yellow]"
            )
            pass
